...
h     = 0.120;  % NACA00xx, h = 0.xx
angle = 0.000;  % attack angle (pi)
...
ncenter(1) =  (center(1)-pivot)*cos(angle)+center(2)*sin(angle)+pivot;
...
%% plot
nx1 =  (x-pivot)*cos(angle)+y*sin(angle)+pivot;
ny1 = -(x-pivot)*sin(angle)+y*cos(angle);
nx2 =  (x-pivot)*cos(angle)-y*sin(angle)+pivot;
ny2 = -(x-pivot)*sin(angle)-y*cos(angle);

plot(nx1, ny1,'r', nx2, ny2,'k','LineWidth',1.2)
